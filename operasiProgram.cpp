...
void tambahBarang(List_parent &LP, List_relasi &LR, List_child &LC, infotype_parent &x)
{
        ...
        if(alamat_parent==NULL)                                          //elemen duplikasi tidak ditemukan
        {
            if(first(LC)!=NULL)                                          //Cek child kosong ato gak
            {
                ...
                else                                                    //jika kategori di child tidak ada yang sesuai dengan kategori inputan
                {
                    ...
                    */
                    cout<<"Maaf kategori tersebut tidak ada, silahkan tambahkan dulu kategori "<<endl;
                    cin.get();
                    tambahKategori(LC,id_kategori);
                }
            }
            else  //child kosong
            {
...
void BarangKeluar(List_parent &LP,List_relasi &LR){
    ...
    if(Cari!=NULL){
        if(Banyak<info(Cari).stok){
            ...
            cout<<"Maaf, stok hanya ada "<<info(Cari).stok<<endl;
            return;
        }
    }else{
